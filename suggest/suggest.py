...
class Suggest(commands.Cog):
        ...
        if str(ctx.author.id) not in self.banlist:
            async with ctx.channel.typing():
                ...
                if config is None:
                    ...
                    review_channel = self.bot.get_channel(
                        int(config["review-channel"]["channel"])
                    )
                    embed = discord.Embed(color=0x59E9FF)
                    embed.set_author(name=f"Suggestion: Waiting for review")
                    ...
                    await ctx.send(f"{ctx.author.mention}, Your suggestion has been submitted for review and will be posted in suggestion channel soon!",delete_after=30)
        else:
            await ctx.send(
                embed=discord.Embed(
    ...
    async def review(self, ctx, msg: int, *, reasonn=None):
        """
        Review an suggestion and send it to suggestion channel.

        **Usage**:
        [p]review 8258382828283
        """
        s_message = None
        for channel in ctx.guild.channels:
            if not isinstance(channel, discord.TextChannel):
                continue
            try:
                s_message = await channel.fetch_message(msg)
        ...
        if config is None:
            ...
            embed1 = s_message.embeds[0]
            try:
               embed1.set_author(name=embed1.author.name.replace(": Waiting for review", f" #{next_id}: Waiting"), icon_url=embed1.author.icon_url, url=embed1.author.url)
            except discord.Error:
               return
            if reasonn == None: 
                message = await suggestion_channel.send("<@&1065965893508681811>",embed=embed1)
            else:
                if reasonn == "minecraft".lower():
                    reasonn = "<@&1066682614909517846>"
                    message = await suggestion_channel.send(f"{reasonn}",embed=embed1)
                else:
                    return await ctx.send("Error!")
            emsgdelete = await s_message.delete()
            await self.coll.find_one_and_update(
             {"_id": "suggestions"},
              {
               "$set": {
    ...
    async def setreviewchannel(self, ctx, *, channel: discord.TextChannel):
        """
        Set the channel where suggestions go for review.

        **Usage**:
        [p]setreviewchannel #review
        [p]src review
        [p]src 515085600047628288
        """
        await self.coll.find_one_and_update(
            {"_id": "config"},
            {"$set": {"review-channel": {"channel": str(channel.id)}}},
            upsert=True,
        )
        embed = discord.Embed(
            title=f"Set review channel to #{channel}.", color=0x4DFF73
        )
        embed.set_author(name="Success!")
        embed.set_footer(text="Task succeeded successfully.")
        await ctx.send(embed=embed)
